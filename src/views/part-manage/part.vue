  ...
  <div>
    ...
    <div class="card" style="margin-bottom: 10px">
      <div style="margin-bottom: 10px">
        <el-button type="primary" @click="handleAdd">新增零件</el-button>
      </div>
      <el-table :data="data.tableData" style="width: 100%">
        ...
        <el-table-column prop="supplierName" label="供应商">
          <template #default="scope">
            <span>{{data.supperListMap[scope.row.supplierId].name}}</span>
          </template>
        </el-table-column>
    ...
    <el-dialog v-model="data.formVisible" title="信息" width="40%" destroy-on-close>
      <el-form :model="data.form" label-width="100px" style="padding-right:50px;">
        <el-form-item label="零件名称">
          <el-input v-model="data.form.name"  placeholder="请输入零件名称" />
        </el-form-item>
        <el-form-item label="图片">
          <el-input v-model="data.form.img"  placeholder="请输入图片地址" />
        </el-form-item>
        <el-form-item label="库存数">
          <el-input v-model="data.form.count"  placeholder="请输入库存数量" />
        </el-form-item>
        <el-form-item label="供应商">
          <el-select v-model="data.form.supplierId" filterable placeholder="请选择供应商">
            <el-option
                v-for="item in data.supplierList"
                :key="item.id"
                :label="item.name"
...
const save = () => {
  request.request({
    ...
    url: data.form.id ? '/part/update' : '/part/add',
    data: data.form
  }).then(res => {
    if (res.code === '200') {
...
const handleDelete = (id) => {
  ElMessageBox.confirm('删除后数据无法恢复，您确定删除吗', '确认删除', {type: 'warning'}).then(() => {
    request.delete('/part/delete/' + id).then(res => {
      if (res.code === '200') {
        ElMessage.success('操作成功')
        load()
      } else {
